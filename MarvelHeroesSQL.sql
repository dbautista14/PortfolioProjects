...
-- Determining what the avg strength, max fighting skills, and min speed Marvel characters have

select * from marvels;
select avg(strength), max(fighting_skills), min(speed) from marvels; 
select name, fighting_skills from marvels group by name having fighting_skills >4;
select name, fighting_skills from marvels group by name having fighting_skills <4;
select name, fighting_skills from marvels group by name having fighting_skills =4;


-- Determining which Marvel characters have above average, average, and minimal fighting skills to determing who is a worthy foe

select name, fighting_skills from marvels;
 select name, case when fighting_skills >4 then "Formidable"
 when fighting_skills <4 then "Weak"
 else "Unworthy" end as "Adversary" from marvels;
 
 
 -- Determining which Marvel characters have the best brains

 select name from marvels where intelligence > 4 and fighting_skills >4;
 select name from marvels where strength > 4 or intelligence > 4;
